# Graph Report - ant  (2026-09-20)

## Corpus Check
- 117 files · ~64,170 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2148 nodes · 3878 edges · 134 communities (122 shown, 7 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 110 edges (avg confidence: 0.82)
...
2. `AntColony.Core` - 61 edges
3. `BuildingBase` - 57 edges
4. `AntColony.Data` - 57 edges
5. `ResourceNode` - 57 edges
6. `BuildingPlacementController` - 56 edges
7. `AntColony.Buildings` - 54 edges
8. `ExpeditionSite` - 52 edges
9. `AntColony.Units` - 49 edges
10. `UnitRole` - 44 edges
...
Cohesion: 0.14
Nodes (12): SettlementDefense, Attackers, CaptureProgress, Prisoners, Remaining, Status, UnderAttack, Collider (+4 more)
...
Cohesion: 0.06
Nodes (29): CombatRolePrototypeBootstrapper, Barracks, GameObject, MenuItem, MonoScript, ResearchLab, Transform, CommanderAcquisitionBootstrapper (+21 more)
...
Cohesion: 0.27
Nodes (3): ColonyInvasion, List, Transform
...
Cohesion: 0.09
Nodes (22): ObjectPool, Dictionary, GameObject, Quaternion, Vector3, UnitData, AntUnitBase, Agent (+14 more)
...
Cohesion: 0.16
Nodes (10): AttackMoveController, ConsumesPointerInput, IsAttackMode, Camera, LayerMask, Vector2, IReadOnlyList, UnitSelectionController (+2 more)
...
Cohesion: 0.16
Nodes (6): EnemyColony, IsDefeated, RemainingBuildings, List, ResourceType, Vector3
...
Cohesion: 0.15
Nodes (12): 2026-09-03~04 (세션 2), 2026-09-10 SAVE — 이전 로그 이관 (2026-09-09 상태), 2026-09-11 — SAVE 연결 복구, 2026-09-11 — 이전 로그 요약 및 중단 지점, 2026-09-13 SAVE — 이전 로그(2026-09-11) 이관, 2026-09-14 SAVE — 이전 로그(2026-09-13) 이관, 2026-09-14 SAVE — 이전 로그(Support 전용 버프) 이관, 2026-09-14 SAVE — 이전 로그(적 소굴 랜덤 배치) 이관 (+4 more)

### Community 65 - "프로젝트 로그"
Cohesion: 0.33
Nodes (5): 검증, 미해결·다음 작업, 저장 상태, 프로젝트 로그, 현재 상태 — 2026-09-20
...
Cohesion: 0.08
Nodes (25): ExpeditionSite, Boss, CanResolveConquest, Cleared, Colony, Defense, Difficulty, Disposition (+17 more)
...
### Community 70 - "BuildingConstructionSite"
Cohesion: 0.23
...
### Community 86 - "UpkeepManager"
Cohesion: 0.16
Nodes (7): CommanderEdgeChecks, Button, Task, MonoBehaviour, Task, SaveProgressionCapture, UpkeepManager
...
Cohesion: 0.14
Nodes (9): SetupInvasion, WildMonster, CurrentHealth, IsDead, IsFlying, Position, List, NavMeshAgent (+1 more)
...
Cohesion: 0.11
Nodes (8): CombatTargeting, Vector3, Vector3, IDamageable, IsDead, Position, Vector3, Vector2
...
Cohesion: 0.22
Nodes (6): AnnexedSettlement, DockedTransport, Garrison, Site, IReadOnlyList, List

### Community 113 - "IsometricCameraController"
Cohesion: 0.20
Nodes (6): IsometricCameraController, FocusPoint, Camera, Vector3, Bounds, AntColony.Camera

### Community 114 - ".CreateSelectionBoxImage"
Cohesion: 0.29
...
### Community 125 - "ConquestDisposition"
Cohesion: 0.40
...
### Community 131 - ".GetTemplate"
Cohesion: 0.28
Nodes (6): Barracks, GameObject, PrisonerCamp, ResearchLab, ScienceLab, ScoutPost

### Community 132 - "UnitRole"
Cohesion: 0.25
Nodes (7): UnitRole, Defense, Flying, Melee, Ranged, Support, Worker

### Community 133 - "LocalIncursions"
Cohesion: 0.33
Nodes (4): LocalIncursions, Visitors, IReadOnlyList, List

## Knowledge Gaps
- **757 isolated node(s):** `AntColony.EditorTools`, `IsCasting`, `IsCasting`, `IsCasting`, `CurrentHp` (+752 more)
...
- **Why does `CommanderAnt` connect `CommanderAnt` to `.TryAssign`, `.Main`, `.Main`, `AntColony.Core`, `UnitRole`, `SoldierAnt`, `SettlementDefense`, `ResourceNode`, `.Main`, `BossHealth`, `CommanderTraits`, `ResearchLab`, `.Main`, `SelectedUnitPanel`, `.Main`, `CommanderSkills`, `NurseryChamber`, `HUDController`, `.Main`, `AntUnitBase`, `.Main`, `.Main`, `ExpeditionTransport`, `.Main`, `ExpeditionSite`, `BuildingConstructionSite`, `SupportChecks`, `UpkeepManager`, `CommanderRank`, `.Main`, `IDamageable`, `CommanderWorkProficiency`, `WorkerAnt`, `AnnexedSettlement`, `.ConfigureCommander`, `CommanderRoster`, `.Main`, `.LateUpdate`, `.SetEmbarked`, `.Main`, `.OnDisable`?**
  _High betweenness centrality (0.116) - this node is a cross-community bridge._
- **Why does `BuildingBase` connect `BuildingBase` to `.Main`, `.Main`, `BuildingPlacementController`, `AntColony.Core`, `.Main`, `ResourceManager`, `ResearchLab`, `.Main`, `BuildingKind`, `.Main`, `MonoBehaviour`, `.Main`, `.Main`, `.GetBuildLabel`, `EnemyColony`, `ExpeditionTransport`, `.Main`, `ExpeditionSite`, `GameManager`, `Barracks`, `QueenChamber`, `ScienceLab`, `.Main`, `.Main`, `Storage`, `IDamageable`, `.SetupFarm`, `WorkerAnt`, `DigSite`, `.Main`, `.TryPlace`, `.Main`?**
  _High betweenness centrality (0.067) - this node is a cross-community bridge._
- **Why does `ResourceNode` connect `ResourceNode` to `.Main`, `.Main`, `.GetTemplate`, `AntColony.Core`, `.Main`, `BossHealth`, `.Main`, `BuildingBase`, `MonoBehaviour`, `.Main`, `.Main`, `.Main`, `EnemyColony`, `ExpeditionTransport`, `.Main`, `.SetMaterial`, `ExpeditionSite`, `.Spawn`, `UpkeepManager`, `ResourceNodeStatus`, `IDamageable`, `.SetupFarm`, `WorkerAnt`, `.Main`?**
